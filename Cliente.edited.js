...
class Client extends EventEmitter {
    ...
    async initialize() {
		...
		//console.log('webdriver fix');
	    browserArgs.push('--disable-blink-features=AutomationControlled');
	    browser = await puppeteer.launch({...puppeteerOpts, args: ["--proxy-server=socks5://127.0.0.1:9050","--no-sandbox", "--disable-setuid-sandbox"]});
        
	
	    var pageTest = await browser.newPage();


await pageTest.goto('https://httpbin.org/ip');

console.log('goto')

   const bodyHTML = await pageTest.evaluate(() => new XMLSerializer().serializeToString(document))
